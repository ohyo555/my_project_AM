			...
			if (cmd.equals("article write")) {
				...
				if (articles.size() == 0) {
					...
					for (int i = articles.size() - 1; i >= 0; i--) {
						Article article = articles.get(i); // articles에서 인덱스에 해당하는 article 가져오는거니까
						
						try {
							if (article.getTitle().contains(cmdDiv[2])) {
								if (Util.getNowDate_TimeStr().split(" ")[0]
										.equals(article.getRegDate().split(" ")[0])) {
